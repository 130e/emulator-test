...
int worker_handle_drop(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
                       u_int32_t id);

int worker_handle_reorder(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
                          u_int32_t id);

int worker_handle_dedup(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
                        u_int32_t id, struct nfq_data *nfa);

int worker_handle_rwnd(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
                       u_int32_t id, struct nfq_data *nfa);

int worker_callback(struct nfq_q_handle *qh, struct nfgenmsg *nfmsg,
  ...
  packet_logging(nfa);
  // handle drop
  if (worker_handle_drop(ctx, &ret, qh, id))
    return ret;
  // handle reorder
  if (worker_handle_reorder(ctx, &ret, qh, id))
    return ret;
  // handle dedup
  if (worker_handle_dedup(ctx, &ret, qh, id, nfa))
    return ret;
  // handle rwnd and update last_ack always
  if (worker_handle_rwnd(ctx, &ret, qh, id, nfa))
    return ret;
  return nfq_set_verdict2(qh, id, NF_ACCEPT,
                          __atomic_load_n(&ctx->mark, __ATOMIC_SEQ_CST), 0,
                          NULL);
}
...
int worker_handle_reorder(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
  ...
  if (ctx->do_reorder) {
    if (ctx->reorder_count-- > 0) {
      ...
      // wait for the offset
      *ret = nfq_set_verdict2(qh, id, NF_ACCEPT,
                              __atomic_load_n(&ctx->mark, __ATOMIC_SEQ_CST), 0,
                              NULL);
    ...
    if (ctx->reorder_offset <= 0) {
      ...
      for (int i = ctx->reorder_id_cur - 1; i >= 0; i--) {
        *ret = nfq_set_verdict2(qh, ctx->reorder_ids[i], NF_ACCEPT,
                                __atomic_load_n(&ctx->mark, __ATOMIC_SEQ_CST),
                                0, NULL);
...
int worker_handle_rwnd(worker_ctx *ctx, int *ret, struct nfq_q_handle *qh,
  ...
  if (ihl < (int)sizeof(struct iphdr) || len < ihl + (int)sizeof(struct tcphdr))
    return 0;
  struct tcphdr *tcph = (struct tcphdr *)(nf_packet + ihl);
  ...
  if (ctx->do_rwnd) {
    ...
    if (ctx->tune_rwnd < original_rwnd) {
      /* log_trace("%u %u", (unsigned short)original_rwnd, */
      /*           (unsigned short)ctx->tune_rwnd); */
      struct pkt_buff *pkBuff = pktb_alloc(AF_INET, nf_packet, len, 0x1000);
      struct iphdr *m_iph = nfq_ip_get_hdr(pkBuff);
      nfq_ip_set_transport_header(pkBuff, m_iph);
      struct tcphdr *m_tcph = nfq_tcp_get_hdr(pkBuff);
      m_tcph->window = htons(ctx->tune_rwnd);
      nfq_tcp_compute_checksum_ipv4(m_tcph, m_iph);
      *ret = nfq_set_verdict2(qh, id, NF_ACCEPT,
                              __atomic_load_n(&ctx->mark, __ATOMIC_SEQ_CST),
...
void worker_set_reorder(worker_ctx *ctx, int enable, uint16_t count,
                        uint16_t offset) {
  ...
  if (enable) {
    ...
    ctx->reorder_id_cur = 0;
    log_debug("worker %s now reordering %hu packets with offset %hu", ctx->id,
              count, offset);
  } else {
