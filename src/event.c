...
int parse_ho(scheduler_ctx *scheduler, action_ctx *action, int time_ms,
             char *data) {
  // handle OUTQ
  char *tok = data;
  ...
  action_stop_nfqueue_args *stop_args =
      malloc(sizeof(action_stop_nfqueue_args));
  stop_args->ctx = action;
  stop_args->queue_num = queue_num;
  scheduler_add_event(scheduler, time_ms, action_stop_nfqueue, stop_args);

  action_start_nfqueue_args *start_args =
      malloc(sizeof(action_start_nfqueue_args));
  start_args->ctx = action;
  start_args->queue_num = queue_num;
  scheduler_add_event(scheduler, time_ms + gap_time, action_start_nfqueue,
                      start_args);
  ...
  // if (false)
  {
    action_set_reorder_args *reord_args =
        malloc(sizeof(action_set_reorder_args));
...
int parse_init(scheduler_ctx *scheduler, action_ctx *action, int time_ms,
               char *data) {
  char *tok = data;
  int queue_num = strtol(tok, NULL, 0);
  tok = strtok(NULL, ",");
...
int parse_handle_dup(scheduler_ctx *scheduler, action_ctx *action, int time_ms,
                     char *data) {
  char *tok = data;
  int queue_num = strtol(tok, NULL, 0);
  tok = strtok(NULL, ",");
...
/*int parse_handle_rw(scheduler_ctx *scheduler, action_ctx *action, int time_ms,
 * char *data) {*/
/*char *tok = data;*/
/*int queue_num = strtol(tok, NULL, 0);*/
/*tok = strtok(NULL, ",");*/
...
int parse_init_sched(scheduler_ctx *scheduler, action_ctx *action, int time_ms,
                     char *data) {
  char *tok = data;
  int queue_num = strtol(tok, NULL, 0);
  tok = strtok(NULL, ",");
...
// data: queue_num enable_freeze freeze_ms enable_dedup smooth_ms burst_duration
// enable_adapt enable_adapt threshold pace
int parse_handle_solution(scheduler_ctx *scheduler, action_ctx *action,
                          int time_ms, char *data) {
  char *tok = data;
  int queue_num = strtol(tok, NULL, 0);
  // mark
  tok = strtok(NULL, ",");
  ...
  if (enable_dedup > 0) {
    ...
    dedup_args = malloc(sizeof(action_set_dedup_args));
    dedup_args->ctx = action;
    dedup_args->enable = false;
    dedup_args->queue_num = queue_num;
    scheduler_add_event(scheduler, time_ms + smooth_ms, action_set_dedup,
                        dedup_args);
  }
  // burst estimation
  if (burst_duration > 0) {
    ...
    burst_args->queue_num = queue_num;
    scheduler_add_event(scheduler, time_ms + burst_duration, action_set_burst,
                        burst_args);
  }
