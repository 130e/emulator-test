...
void action_init_ctx(action_ctx *ctx) {
  ...
  ctx->workers =
      (worker_ctx **)calloc(ACTION_MAX_NFQUEUE, sizeof(worker_ctx *));
}
...
void tcp_schedule(void *data) {
  ...
  if (ctx->do_burst) {
    ...
    double rate = ((double)tctx->hist_param[tctx->mark].rtt /
                   (double)tctx->sample_interval);
  ...
  /*log_fatal("sched %d %d %d", ctx->tune_rwnd, ctx->hist_param[1].hist_rw,
   * ctx->hist_param[2].hist_rw);*/
...
void action_init_nfqueue(void *data) {
  ...
  if (nfq_init(qctx, args->queue_num, worker_callback, wctx)) {
    log_fatal("failed to initialize nfqueue %d: %s", args->queue_num,
              strerror(errno));
    free(qctx);
    free(wctx);
    exit(1);
...
void action_teardown_nfqueue(void *data) {
  ...
  if (nfq_teardown(qctx)) {
    log_error("failed to teardown nfqueue %d: %s", args->queue_num,
              strerror(errno));
    return;
  }
