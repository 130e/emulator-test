...
int scheduler_add_event(scheduler_ctx *ctx, uint64_t time, scheduler_handler h,
                        void *data) {
...
int scheduler_compare_event(const void *a, const void *b) {
  if (((scheduler_event *)a)->time_ms > ((scheduler_event *)b)->time_ms) {
    return 1;
  } else if (((scheduler_event *)a)->time_ms <
             ((scheduler_event *)b)->time_ms) {
    return -1;
...
void scheduler_run(scheduler_ctx *ctx, tcp_ctx *tcp_sched) {
  ...
  qsort(ctx->events, ctx->event_count, sizeof(scheduler_event),
        scheduler_compare_event);
  ...
  while (i < ctx->event_count) {
    ...
    if (ctx->events[i].time_ms < fire) {
      ...
      i++;
      log_trace("scheduler %s: executing event %d at %ld ms", ctx->id, i,
                (long)fire);
    } else {
    ...
    event_handler(data);
    log_debug("scheduler %s: Finish executing event %d at %ld ms", ctx->id, i,
              (long)fire);
  }
